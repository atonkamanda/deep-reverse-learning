...
class CNN_MNIST(nn.Module):
    ...
    def forward(self, x):
        x = self.conv1(x)
        x = F.relu(x)
        k_1, _ = x.flatten(start_dim=1).topk(32, dim=1)
        x = F.max_pool2d(x, 2)
        x = self.conv2(x)
        x = F.relu(x)
        k_2, _ = x.flatten(start_dim=1).topk(32, dim=1)
        x = F.max_pool2d(x, 2)
        x = x.view(-1, 320)
        x = self.fc1(x)
        x = F.relu(x)
        k_3, _ = x.topk(32, dim=1)
        x = self.fc2(x)
        x = F.softmax(x, dim=1)
        
        # Compute entropy of the output distribution
        K = torch.cat((k_1,k_2,k_3),dim=0)
    ...
    for e in range(epoch):
        if model.wake == True:
            ...
            for i in range(17000):
                # Make proportional the loss to activation_value*n_activations 
                noise_injection = torch.randn(1,28,28)
...
optimizer2 = optim.SGD(model.parameters(), lr=0.0001, momentum=0.5)
"""optimizer = optim.SGD([
